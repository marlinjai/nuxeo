...
public class ConfigurationMarshaller {
    ...
    protected boolean isConfigurationValueEquals(String userValue, String valueToSet) {
        // userValue is a raw nuxeo.conf line, meaning it can contain \\u character
        // whereas configHolder#getRawProperty has an unescape version of this line, \\u are resolved
        // so unescape the \\u character for the check
        return userValue != null
                && (userValue.equals(valueToSet) || StringEscapeUtils.unescapeJava(userValue).equals(valueToSet));
    }
    ...
    protected void storeNewFilesList(ConfigurationHolder configHolder, List<String> newFilesList) throws IOException {
        Path newFiles = configHolder.getTemplatesPath().resolve(NEW_FILES);
        try (BufferedWriter writer = Files.newBufferedWriter(newFiles, APPEND, CREATE, WRITE)) {
    ...
    protected void dumpProperties(ConfigurationHolder configHolder) {
        ...
        try {
            ...
            try (var os = Files.newBufferedWriter(dumpedFile)) {
